//my plotting tool
//Plotting tool for Nishank's study
#include "TGraph.h"
...
void process_data_gauss(int min, int max, string compare_dummy, vector<TString> eq_dummy, vector<TString> bkg_names_dummy, int bkg_names_size, double mean_arr[], int mean_size, string data1_dummy, string data2_dummy, string data3_dummy, string type, string plot_name, string spur_type, string rootName1, string rootName2, string rootName3, int fold){
	...
	for(int k = 0; k < mean_size; k++){
		...
		//rename
			...
			integral = fixHist1 -> Integral();
				...
				TCanvas *c = new TCanvas(Form("%s", bkg_names_dummy[i].Data()), Form("%s", bkg_names_dummy[i].Data()), canW, canH);
				c->SetWindowSize(winW, winH);
				...
				l->AddEntry("gauss_ss", Form("%s, Width: %.2f #pm %.2f, Mean: %.2f #pm %.2f", data1_dummy.c_str(), width_ss.getVal(), width_ss.getError(), mean_ss.getVal(), mean_ss.getError()), "L");
				l->AddEntry("gauss_ss1", Form("%s, Width: %.2f #pm %.2f, Mean: %.2f #pm %.2f", data2_dummy.c_str(), width_ss1.getVal(), width_ss1.getError(), mean_ss1.getVal(), mean_ss1.getError()), "L");
				l->AddEntry("gauss_ss2", Form("%s, Width: %.2f #pm %.2f, Mean: %.2f #pm %.2f", data3_dummy.c_str(), width_ss2.getVal(), width_ss2.getError(), mean_ss2.getVal(), mean_ss2.getError()), "L");
				...
				l->AddEntry("ss", Form("Data %s", data1_dummy.c_str()), "LEP");
				l->AddEntry("ss1", Form("Data %s", data2_dummy.c_str()), "LEP");
				l->AddEntry("ss2", Form("Data %s", data3_dummy.c_str()), "LEP");
    ...
    for (int j = 0; j < mean_arr.size(); j++) {
        for(int i = 0; i < bkg_names_size; i++){
            //mean
            mean_mc[j][i] = mean_0[j][i];
            mean_error_mc[j][i] = mean_Err0[j][i];

            mean_nf[j][i] = mean_1[j][i];
            mean_error_nf[j][i] = mean_Err1[j][i];

            mean_truth[j][i] = mean_2[j][i];
            mean_error_truth[j][i] = mean_Err2[j][i];

            //width

            width_mc[j][i] = width_0[4][i];
            width_error_mc[j][i] = width_Err0[4][i];

            width_nf[j][i] = width_1[4][i];
            width_error_nf[j][i] = width_Err1[4][i];

            width_truth[j][i] = width_2[4][i];
            width_error_truth[j][i] = width_Err2[4][i];
        }
    }
   ...
   c1 -> SaveAs(Form("distributions/%s/%s_scatter.png", compare_dummy.c_str(), type.c_str()));
	...
	h3->SetStats(0);
	h5->SetStats(0);
	h1->GetYaxis()->SetRangeUser(ml, mr);
	...
	h4->SetStats(0);
	h6->SetStats(0);
	h2->GetYaxis()->SetRangeUser(wl, wr);
...
void plot_Nishank(float value, int fold_value){
